...
public class Contest extends BaseEntity{
    ...
    private void validateQuestionList(List<Question> qList, Level contestLevel) throws InvalidContestException {
        // Level firstQuesList = qList.get(0).getLevel();

        for(Question question : qList){
            // if(question.getLevel() != firstQuesList){
            //     throw new InvalidContestException("Questions do not have the same level as the contest level");
            // }
            if(question.getLevel()!=contestLevel){
                throw new InvalidContestException("Questions are not at the contest level");
            }
        }
    }
