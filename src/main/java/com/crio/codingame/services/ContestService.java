...
public class ContestService implements IContestService {
    ...
    if (numQuestion >= questions.size()) {
        // Return all questions if the requested number is greater than or equal to the total number of questions
        return questions;
    } else {
        ...
        for (int i = 0; i < numQuestion; i++) {
            int randomIndex = random.nextInt(copyOfQuestions.size());
            pickedQuestions.add(copyOfQuestions.remove(randomIndex));
        }
    ...
    //  return Collections.emptyList();
    if (level == null) {
        // If level is not specified, return all contests
        return contestRepository.findAll();
    } else {
        // Return contests that match the provided level
